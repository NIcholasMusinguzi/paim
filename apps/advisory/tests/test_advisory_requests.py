...
def test_request_appears_in_the_shared_parish_queue_for_chief_and_agent(world):
    _client(world["farmer_user"]).post("/api/v1/advisory-requests/mine/", {"message": "Help"}, format="json")

    chief_queue = _client(world["chief"]).get("/api/v1/advisory-requests/queue/")
    assert len(chief_queue.data) == 1

    agent_queue = _client(world["agent"]).get("/api/v1/advisory-requests/queue/")
    assert len(agent_queue.data) == 1

    other_queue = _client(world["other_chief"]).get("/api/v1/advisory-requests/queue/")
...
@pytest.mark.django_db
def test_non_farmer_cannot_submit_a_request(world):
    res = _client(world["chief"]).post("/api/v1/advisory-requests/mine/", {"message": "x"}, format="json")
    assert res.status_code == 403
