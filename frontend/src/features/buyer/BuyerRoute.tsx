...
function BidForm({ lotId }: { lotId: number }) {
  ...
  return (
    <form onSubmit={onSubmit} className="flex flex-col gap-3 rounded-lg bg-panel p-4">
      <h3 className="text-sm font-semibold uppercase tracking-wide text-soft">Submit a sealed bid</h3>
      <Field label="Price (UGX/kg)" type="number" min={1} required value={price}
        onChange={(e) => setPrice(e.target.value)} />
      <Field label="Terms" required value={terms} onChange={(e) => setTerms(e.target.value)} />
      {submit.isError && (
...
function LotDetailPanel({ lotId, onClose }: { lotId: number; onClose: () => void }) {
      ...
      {lot.status === "open" ? (
        myBid ? (
          <p className="text-sm text-ink">
            Your bid: <span className="tabular">UGX {myBid.price_per_kg}/kg</span> — sealed until the lot closes. You
            cannot see other buyers' bids, and they cannot see yours.
          </p>
        ) : (
          <BidForm lotId={lotId} />
        )
        ...
        <div className="flex flex-col gap-2">
          <h3 className="text-sm font-semibold uppercase tracking-wide text-soft">Bids (disclosed at closure)</h3>
          {lot.bids.length === 0 ? (
            ...
            <ul className="flex flex-col gap-1">
              {lot.bids.map((b) => (
                <li key={b.id} className="flex items-center justify-between text-sm">
                  ...
                  <span className="tabular text-ink">UGX {b.price_per_kg}/kg</span>
                </li>
              ))}
            </ul>
          )}
...
function BuyerRoute() {
  ...
  return (
    <div className="mx-auto flex max-w-2xl flex-col gap-4 p-4">
      ...
      {selected != null && <LotDetailPanel lotId={selected} onClose={() => setSelected(null)} />}
